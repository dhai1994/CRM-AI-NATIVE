...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      <div style={{ width: "220px", minHeight: "100vh", height: "100%", background: "#0a0a0a", borderRight: "1px solid #1a1a1a", padding: "24px 16px", display: "flex", flexDirection: "column", fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif", flexShrink: 0, position: "sticky", top: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "32px", paddingLeft: "4px", animation: "slideIn 0.3s ease both" }}>
          <LogoMark />
          <div>
        ...
        <p style={{ fontSize: "10px", fontWeight: 700, color: "#333", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: "8px", paddingLeft: "12px" }}>
          Menu
        </p>
          ...
          {NAV_ITEMS.map(({ to, label, icon, accent }, i) => {
            ...
            return (
              <NavLink
                key={to}
                to={to}
                className={({ isActive: navActive }) => "crm-sidebar-link" + (navActive ? " active" : "")}
                style={{ animation: `slideIn 0.3s ease ${i * 0.05}s both`, ...(isActive ? { color: accent } : {}) }}
              >
                {isActive && (
                  <div style={{ position: "absolute", left: 0, top: "6px", bottom: "6px", width: "3px", borderRadius: "0 3px 3px 0", background: accent, marginLeft: "-1px" }} />
                )}

                <span className="nav-icon">{icon}</span>
                <span style={{ flex: 1 }}>{label}</span>
                <span className="active-dot" style={{ background: accent }} />
              </NavLink>
            );
          })}
        </nav>
